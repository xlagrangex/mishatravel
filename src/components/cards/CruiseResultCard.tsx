...
import { Ship, Clock, Waves, Calendar } from "lucide-react";
...
export default function CruiseResultCard({
  ...
  const departurePrices = departures
    .filter((d) => d.data_partenza >= today && d.prezzo_main_deck != null && d.prezzo_main_deck > 0)
    .map((d) => d.prezzo_main_deck as number);
        ...
        <div className="flex-1 p-4 md:p-5 flex flex-col">
          <div className="flex-1">
            <h3 className="font-bold text-lg text-gray-900 font-[family-name:var(--font-poppins)] group-hover:text-[#C41E2F] transition-colors line-clamp-2">
              <Link href={`/crociere/${slug}`}>{title}</Link>
            </h3>

            <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-500">
              <span className="flex items-center gap-1"><Ship className="size-3.5" />{shipName}</span>
              <span className="flex items-center gap-1"><Waves className="size-3.5" />{river}</span>
