...
const cruiseSchema = z.object({
  ...
  prezzo_listino: z.coerce.number().nullable().default(null),
...
export async function saveCruise(formData: unknown): Promise<ActionResult> {
  ...
  const cruiseRow = {
    ...
    prezzo_listino: data.prezzo_listino || null,
    prezzo_offerta: data.prezzo_offerta || null,
  ...
  try {
    ...
    const { data: inserted, error: insertError } = await supabase
      ...
      .insert({
        ...
        price_type: cruise.price_type ?? 'da',
        prezzo_listino: cruise.prezzo_listino,
        prezzo_offerta: cruise.prezzo_offerta,
